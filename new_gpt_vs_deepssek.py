...
def build_system_prompt(topic: str, traits: dict) -> str:
    return (
        ...
        f"You are a debate expert person participating in a structured debate on: '{topic}\n"
...
def save_debates(output_file: str = "final_discorse_gptvsdeepseek.json"):
    """Saves debates incrementally after each topic-trait combination."""
    ...
    for traits in TRAITS:
        for topic in TOPICS:
            ...
            new_debates.append(new_entry)

            # **Save after every topic-trait debate**
            try:
                with open(output_file, "w", encoding="utf-8") as f:
                    json.dump(existing_data + new_debates, f, indent=2, ensure_ascii=False)
                print(f"✅ Debate saved: {topic} | Traits: {traits}")
            except Exception as e:
                print(f"❌ ERROR saving debate log: {str(e)}")
